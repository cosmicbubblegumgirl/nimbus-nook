(() => {
  ...
  const THEME_KEY = "nimbus-nook-theme";
  ...
  const journalLayouts = [
    {
      name: "Worry Map",
      category: "Thought",
      layoutClass: "layout-map",
      description: "Turn a spiraling worry into facts, needs, and one next step.",
      prompts: ["The worry says", "What I know right now", "What I need", "One next step"]
    },
    {
      name: "Evidence Lantern",
      category: "Thought",
      layoutClass: "layout-lantern",
      description: "Light up anxious predictions with balanced evidence.",
      prompts: ["Prediction", "Evidence for", "Evidence against", "Balanced thought"]
    },
    {
      name: "Body Weather",
      category: "Grounding",
      layoutClass: "layout-weather",
      description: "Track sensations without treating them like emergencies.",
      prompts: ["Body forecast", "Intensity", "Comfort cue", "What changed"]
    },
    {
      name: "Tiny Brave Step",
      category: "Courage",
      layoutClass: "layout-ladder",
      description: "Build a gentle exposure step that feels possible today.",
      prompts: ["Fear story", "Smallest step", "Recovery plan", "Proof I can keep"]
    },
    {
      name: "Safe Place Scene",
      category: "Grounding",
      layoutClass: "layout-scene",
      description: "Write a sensory place your nervous system can visit.",
      prompts: ["I can see", "I can hear", "I can feel", "This place protects me by"]
    },
    {
      name: "Future Me Letter",
      category: "Care",
      layoutClass: "layout-letter",
      description: "Let a steadier version of you speak with tenderness.",
      prompts: ["Dear current me", "This feeling is allowed", "You have already survived", "The kind next thing"]
    },
    {
      name: "Thought Cloud Release",
      category: "Thought",
      layoutClass: "layout-clouds",
      description: "Notice an intrusive thought and practice letting it pass.",
      prompts: ["Thought headline", "Label", "What I will not debate", "Letting it pass looks like"]
    },
    {
      name: "Aftercare Check",
      category: "Recovery",
      layoutClass: "layout-aftercare",
      description: "Recover after people, panic, conflict, or overstimulation.",
      prompts: ["What happened", "What I am replaying", "What went okay", "What I choose to release"]
    },
    {
      name: "Control Circles",
      category: "Thought",
      layoutClass: "layout-circles",
      description: "Sort control, influence, and release without fighting reality.",
      prompts: ["I control", "I influence", "I release for now", "Support I can ask for"]
    },
    {
      name: "Comfort Vault",
      category: "Care",
      layoutClass: "layout-vault",
      description: "Collect reliable comforts for hard moments.",
      prompts: ["Sound", "Texture", "Warmth", "Person or place", "Tiny reward"]
    },
    {
      name: "Decision Compass",
      category: "Courage",
      layoutClass: "layout-compass",
      description: "Make a choice from values instead of panic.",
      prompts: ["Options", "Fear wants", "My value says", "Good-enough choice"]
    },
    {
      name: "Sleep Landing",
      category: "Recovery",
      layoutClass: "layout-sleep",
      description: "Close loops and give tomorrow a container.",
      prompts: ["Unfinished loops", "Tomorrow container", "Body wind-down", "Bedtime kindness"]
    },
    {
      name: "Cupcake Victory",
      category: "Care",
      layoutClass: "layout-victory",
      description: "Celebrate invisible anxiety work with a soft win receipt.",
      prompts: ["Soft win", "Sprinkle detail", "What it proves", "How I will honor it"]
    }
  ];
  ...
  const journalThemes = [
    {
      ...
      description: "iridescent calm",
      accent: "#3a9b9b",
      paper: "#f7fdff",
      line: "rgba(58, 155, 155, 0.14)",
      ink: "#20323a"
    },
    {
      name: "Meadow Grid",
      ...
      accent: "#6aa86d",
      paper: "#fbfff5",
      line: "rgba(106, 168, 109, 0.16)",
      ink: "#243329"
    },
    {
      name: "Moonlit Ink",
      themeClass: "theme-moon",
      description: "night notebook",
      accent: "#8267a6",
      paper: "#f8f4ff",
      line: "rgba(130, 103, 166, 0.16)",
      ink: "#2c2636"
    },
    {
      name: "Citrus Desk",
      themeClass: "theme-citrus",
      description: "warm clarity",
      accent: "#d99a2b",
      paper: "#fffaf0",
      line: "rgba(217, 154, 43, 0.17)",
      ink: "#3b3020"
    },
    {
      name: "Rose Quartz",
      ...
      accent: "#d97988",
      paper: "#fff7fa",
      line: "rgba(217, 121, 136, 0.15)",
      ink: "#3a2830"
    },
    {
      name: "Ocean Current",
      themeClass: "theme-ocean",
      description: "deep breath blue",
      accent: "#2f84a6",
      paper: "#f3fbff",
      line: "rgba(47, 132, 166, 0.16)",
      ink: "#1f3340"
    },
    {
      name: "Sunrise Ledger",
      themeClass: "theme-sunrise",
      description: "hopeful morning",
      accent: "#ee7b6a",
      paper: "#fff9f3",
      line: "rgba(238, 123, 106, 0.16)",
      ink: "#3a2c28"
    },
    {
      name: "Lavender Rain",
    ...
    },
    {
      ...
      accent: "#2b9d83",
      paper: "#f5fffb",
      line: "rgba(43, 157, 131, 0.17)",
      ink: "#213932"
    },
    {
      name: "Cozy Cocoa",
      themeClass: "theme-cocoa",
      description: "grounded warmth",
      accent: "#9b7653",
      paper: "#fff9f1",
      line: "rgba(155, 118, 83, 0.15)",
      ink: "#382d25"
    }
  ];

  const journalTemplates = journalThemes.flatMap((theme, themeIndex) =>
    journalLayouts.map((layout, layoutIndex) => ({
      id: themeIndex * journalLayouts.length + layoutIndex,
      title: `${theme.name} ${layout.name}`,
  ...
  const templateCategories = ["all", ...Array.from(new Set(journalLayouts.map((layout) => layout.category)))];
  ...
  function bindTheme() {
    ...
    setTheme(saved === "cyber" ? "cyber" : "light");
    $("#themeToggle").addEventListener("click", () => {
      setTheme(state.theme === "cyber" ? "light" : "cyber");
    });
  }
  ...
  function setTheme(theme) {
    ...
    if (toggle) {
      toggle.textContent = theme === "cyber" ? "Day" : "Cyber";
      toggle.setAttribute("aria-pressed", String(theme === "cyber"));
    }
  }
  ...
  function bindJournal() {
    $("#journalCover").addEventListener("click", async () => {
      ...
      window.setTimeout(() => {
        ...
        $("#journalStudio").classList.add("revealed");
        cover.hidden = true;
        $("#journalBody").focus();
        $("#journalStudio").scrollIntoView({ behavior: "smooth", block: "start" });
      }, 560);
    });
    $("#newPageButton").addEventListener("click", () => createJournalPage(state.selectedTemplateId, true));
    $("#saveJournalPage").addEventListener("click", saveCurrentPage);
    $("#templateSearch").addEventListener("input", (event) => {
      state.templateSearch = event.target.value.trim().toLowerCase();
      renderTemplates();
    });
    $("#shuffleTemplate").addEventListener("click", () => {
      const random = journalTemplates[Math.floor(Math.random() * journalTemplates.length)];
      applyTemplateToCurrentPage(random.id);
    });
    $("#sparkPrompt").addEventListener("click", insertPromptSpark);
    $("#focusJournal").addEventListener("click", () => {
      state.focusJournal = !state.focusJournal;
      $("#journalStudio").classList.toggle("focus-mode", state.focusJournal);
      $("#focusJournal").textContent = state.focusJournal ? "Studio" : "Focus";
      $("#journalBody").focus();
    });
    ...
    $("#journalCanvas").addEventListener("click", (event) => {
      if (state.selectedSticker) {
        placeSelectedSticker(event);
        return;
      }
      if (!event.target.closest(".placed-sticker")) $("#journalBody").focus();
    });
  ...
  async function createJournalPage(templateId = state.selectedTemplateId, shouldSelect = true) {
    const template = journalTemplates[templateId] || journalTemplates[0];
    const page = {
      userId: state.user.id,
  ...
  function renderTemplateFilters() {
    ...
    templateCategories.forEach((category) => {
      ...
      button.classList.toggle("active", state.templateFilter === category);
      button.addEventListener("click", () => {
        state.templateFilter = category;
        renderTemplateFilters();
        renderTemplates();
      });
      holder.append(button);
  ...
  function renderTemplates() {
    ...
    const visible = journalTemplates.filter((template) => {
      const matchesCategory = state.templateFilter === "all" || template.category === state.templateFilter;
      const haystack = `${template.title} ${template.description} ${template.category} ${template.themeName} ${template.layoutName}`.toLowerCase();
      return matchesCategory && (!query || haystack.includes(query));
    });
    $("#templateStats").textContent = `${visible.length} of ${journalTemplates.length} designs showing`;
    visible.forEach((template) => {
      ...
      button.style.setProperty("--template-paper", template.paper);
      button.classList.toggle("active", state.currentPage && state.currentPage.templateId === template.id);
      button.innerHTML = `
        <span class="template-swatch" aria-hidden="true"></span>
        <span>
          <strong>${escapeHTML(template.layoutName)}</strong>
          <small>${escapeHTML(template.themeName)} &middot; ${escapeHTML(template.category)}</small>
        </span>
      `;
      button.addEventListener("click", () => applyTemplateToCurrentPage(template.id));
      holder.append(button);
    });
    if (!visible.length) {
      holder.innerHTML = '<p class="soft-note">No designs match that search yet.</p>';
    }
  }

  async function applyTemplateToCurrentPage(templateId) {
    if (!state.currentPage) return;
    const template = journalTemplates[templateId] || journalTemplates[0];
    const previousTemplate = journalTemplates[state.currentPage.templateId] || journalTemplates[0];
    const currentText = $("#journalBody").innerText.trim();
    const previousGenerated = bodyFromTemplate(previousTemplate).trim();
    const canReplaceBody = !currentText || currentText === previousGenerated;
    state.selectedTemplateId = template.id;
    state.currentPage.templateId = template.id;
    state.currentPage.title = canReplaceBody ? template.title : state.currentPage.title;
    state.currentPage.body = canReplaceBody ? bodyFromTemplate(template) : $("#journalBody").innerText;
    $("#journalCanvas").classList.remove("page-flip");
    renderJournalPage();
    requestAnimationFrame(() => $("#journalCanvas").classList.add("page-flip"));
    await saveCurrentPage();
  }

  function insertPromptSpark() {
    if (!state.currentPage) return;
  ...
  function renderPageList() {
    ...
    state.journalPages.forEach((page) => {
      ...
      button.innerHTML = `${escapeHTML(page.title)}<small>${date} &middot; before ${page.anxietyBefore} / after ${page.anxietyAfter}</small>`;
  ...
  function renderJournalPage() {
    ...
    $("#activeTemplateName").textContent = `${template.themeName} · ${template.layoutName}`;
    $("#journalBody").innerText = state.currentPage.body || "";
    $("#savedState").textContent = "saved";
    const canvas = $("#journalCanvas");
    canvas.className = `journal-canvas ${template.themeClass} ${template.layoutClass}`;
    canvas.style.setProperty("--paper-bg", template.paper);
    canvas.style.setProperty("--paper-accent", template.accent);
    canvas.style.setProperty("--paper-line", template.line);
    canvas.style.setProperty("--paper-ink", template.ink);
    $$(".template-button").forEach((button) => {
      button.classList.toggle("active", Number(button.dataset.templateId) === template.id);
    });
    renderPlacedStickers();
  }
